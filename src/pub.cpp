...
{ # callback 함수를 정의합니다.
    static int count; # 정적 변수를 선언합니다.
    auto message = std_msgs::msg::String(); # String 타입의 객체를 생성합니다.
    message.data = "Hello world! " + std::to_string(count++); # 메시지의 데이터를 생성합니다.
    RCLCPP_INFO(node->get_logger(), "node Publish: %s", message.data.c_str()); # RCLCPP_INFO를 사용하여 메시지를 출력합니다.
...
int main(int argc, char* argv[]) # main 함수
{
    ...
    auto node = std::make_shared<rclcpp::Node>("node_pub1_1"); # 노드 객체를 생성합니다.
    auto qos_profile = rclcpp::QoS(rclcpp::KeepLast(10)); # publish에 사용할 QoS 프로파일을 설정합니다.
    auto pub = node->create_publisher<std_msgs::msg::String>("/topic_psub1_1", qos_profile); # 발행자를 생성합니다.
    std::function<void()> fn = std::bind(pub_callback, node, pub); # publish함수를 bind하여 실행할 함수 객체를 생성합니다.
    auto timer = node->create_wall_timer(100ms, fn); # 타이머를 생성합니다.
